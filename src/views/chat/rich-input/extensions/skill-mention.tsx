  ...
  addOptions() {
    return {
      suggestion: {
        ...
        // nothing) instead of letting `RichInput`'s own Enter-to-submit
        // handling run. Deactivating the picker for this exact match falls
        // back to normal text/Enter behavior, the same as if `skillsEnabled`
        // were `false` for this one input.
        allow: ({ state, range }) => {
          const text = state.doc.textBetween(range.from, range.to);
          return !isCompactCommand(text);
        },
