...
- **API `usage` is authoritative** for turn-to-turn accounting; local estimation is a conservative gap-filler only (first send + pre-send delta), never used to _skip_ a budget check.
...
- [ ] **Step 5: Remove the dependency.** Delete `llama-cpp-2` from `Cargo.toml`; `cargo build` to prune `Cargo.lock`. Fix every compile error from the removed surface (the compiler lists them). The dialect _render_ pair (`render_tool_use`/`render_tool_result`) is now unused for counting — if it has no other caller, delete it (this is the counting-path half of ToolDialect; `call_format_instructions` stays until SP3). Confirm via `rg`.
