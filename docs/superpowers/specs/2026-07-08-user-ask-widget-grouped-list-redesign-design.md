...
- `RichInput` itself (the text-mode fallback) is untouched — it already has the matching card/shadow/send-button treatment this redesign gives the options module, which is exactly why the fix in Section 1 is to stop double-wrapping it: text mode's own outer box is removed so `RichInput`'s single card is the only border, instead of nesting inside a second one. The header row (Section 1) is unified into one shared structure used by both modes, with a new icon-only back-arrow button replacing today's `"Back to options"` text button, so it shares the exact same slot as options mode's ✕ button and the header never reflows switching modes.

## Section 1: Shell & header (correction from an earlier draft of this doc: this DOES change, more than first stated)

**This section originally claimed the shell was unchanged. That was wrong twice over — caught while re-verifying against both the actual shipped code and the approved mockup before writing the implementation plan.** Today's shipped `UserAskWidget` has two *different* header layouts, one per mode:

- Options mode: `<div className="mb-1 flex items-start gap-2">` containing an optional `detail.header` eyebrow `<p>` and an icon-only ✕ `Button` (`size="icon-sm"`, `aria-label="Close question"`), both inside the single outer bordered box that also contains everything else.
- Text mode: a *different* `<div className="mb-2 flex items-center justify-between gap-2">` containing an `"Answering: {detail.question}"` `<p>` and a full **text** `Button` reading `"Back to options"` (`size="sm"`, no icon) — inside that same single outer bordered box.

The approved design (confirmed against the live mockup) replaces both of these with **one shared header structure**, used identically by both modes, and removes the single outer bordered box entirely:

- The root element becomes a plain, unboxed `flex flex-col` container (no border/background of its own) — `data-testid="user-ask-widget"` moves here from the (removed) outer box.
- Directly inside it, one header row: an eyebrow line (`detail.header`, shown only in options mode) above a single main text line, plus one `ml-auto` **icon-only** button in the same slot in both modes.
  - Options mode: eyebrow shown (if `detail.header` is non-empty); main line is `detail.question`; icon is ✕ (`aria-label="Close question"`).
  - Text mode: eyebrow hidden; main line becomes `` `Answering: ${detail.question}` `` (same wording as today's text-mode header, now sharing the *same* element/styling as the question line instead of a separately-styled paragraph); icon becomes a back-arrow (`aria-label="Back to options"`) — a new icon-only button, replacing today's `"Back to options"` text button. `data-testid="question-back-to-options"` stays on this button (now icon-only).
  - Because both modes render the exact same header markup with only text/icon content swapped, the header never reflows switching modes — this is also what makes the Section 4 mode-switch transition clean (the header region doesn't need to be part of the transitioned area at all).
- **Module** (Section 2) — the *only* bordered surface, directly beneath the header, swapping its content between the options list (options mode) or a bare `RichInput` (text mode — `RichInput` already renders its own card; wrapping it in a second bordered box, as today's code does, is the card-in-a-card bug this pass fixes. `RichInput` itself is unchanged).
...
The pre-existing standalone multi-select hint paragraph (`{detail.multiSelect && <p data-testid="multi-select-indicator">Select all that apply</p>}`, currently rendered between the question and the options) is removed entirely, not just relocated — this was confirmed live in the approved mockup and its changelist ("static 'Select all that apply' line + bottom button → no standing hint text at all now"). The footer's live "`N selected`" count plus the submit button's disabled/enabled state are the only multi-select-state signal going forward; there is no replacement static hint anywhere.
