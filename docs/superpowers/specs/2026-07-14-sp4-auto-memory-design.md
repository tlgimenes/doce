...
**Where:** a new `# Memories` section folded into `plan_system_message` (`src-tauri/src/commands/agent.rs`), in the **same slot** as SP3's `project_instructions_section` (the `AGENTS.md` injection) — after the cwd line, as part of the immutable system prompt. Being in `messages[0]` places it structurally *outside* the compaction window: it is never summarized away.

**KV-prefix stability — narrower than AGENTS.md's.** An earlier draft of this spec claimed the block is "byte-stable per conversation so the KV prefix survives turn boundaries", i.e. the same invariant SP3's AGENTS.md block preserves. **That is false, and the difference matters.** AGENTS.md is a file that does not change under us; the memory set is rewritten by extraction. `replace_memories` re-inserts *every* row with a fresh UUID and one identical `updated_at`, so `ORDER BY updated_at DESC, id` collapses to insertion order = the extraction model's emission order for that pass. A later pass that emits the same facts in a different order therefore produces **different bytes** for the same logical set. What is actually true:

- **Stable *within* one `send_agent_message`:** memories are fetched once and the rendered block is cloned into `messages[0]` for every inner turn, so the prefix holds across a turn's tool-call loop — which is where it matters most.
- **NOT stable across calls** once the workspace's set is rewritten.

The cost is near-zero in the common case: the pass that rewrites the set is *this conversation's own* compaction, which just replaced the history anyway, so the prefix was already gone. The real (and accepted) cost is cross-conversation: a **sibling** conversation in the same workspace compacting invalidates *this* conversation's KV prefix from the memories block onward, even though nothing this conversation did changed.
...
- New helper `memories_section(conn, workspace_id) -> Option<String>`:
  - `SELECT content FROM memories WHERE workspace_id IS ?1 ORDER BY updated_at DESC, id` (use `IS` so a NULL workspace matches the NULL bucket). Note that `updated_at DESC` never actually discriminates: `replace_memories` stamps every row of the swapped-in set with the same `now`, so the real ordering is the `id` tiebreak — see the "emission order" note below.
  - Render as `# Memories\n\n- <content>\n- <content>\n...`.
...
4. Parse the model's output into a new memory set (output contract: one fact per line, or a fenced list — kept dead simple). Each line gets a conservative **shape check** (`context::is_plausible_fact`): a trailing `:` means it's a preamble/header, not a fact, and lines outside `MEMORY_FACT_MIN_CHARS..=MEMORY_FACT_MAX_CHARS` aren't sentence-shaped. This exists because a persisted line is *durable, sticky and self-reinforcing*: the next pass feeds it back under "keep the existing ones that are still true", the model keeps it, and it rides in `messages[0]` forever — there is no UI and no clear command, so the only recovery is manual sqlite surgery. Bias: when in doubt, drop. If a **majority** of non-empty lines fail the check, treat the whole pass as a **parse failure → "no change"**, log, return: this is a *full-set swap*, and a model that mostly emitted garbage cannot be trusted to have emitted a faithful full set, so persisting the surviving minority would silently drop every real memory it failed to re-emit.
5. **Guard, then compare-and-swap the set** (mirroring `evaluate_summary`'s defensive posture): if the parsed set is empty/degenerate *and* existing memories are non-empty, **do nothing** (never let a bad extraction wipe good memories — the unsafe direction). Otherwise `replace_memories_if_unchanged(conn, workspace_id, expected, contents, now)`, where `expected` is the set step 2 actually read. In a single transaction it re-reads the workspace's current contents, compares them to `expected` as an **order-insensitive multiset**, and only then does `DELETE FROM memories WHERE workspace_id IS ?` + insert the new set with fresh `updated_at`; a mismatch writes nothing and returns `Ok(false)` (log, return). Preserve `created_at` for facts whose `content` is unchanged (so "age" survives re-extraction).

   **Why the CAS, not a blind write:** step 2's read and step 5's write straddle a multi-second LLM round-trip, and two conversations in one workspace can compact concurrently (`commands/context.rs`'s manual "Compact now" vs. `agent.rs`'s `maybe_compact`; `ActiveGenerations` is per-conversation, with no global turn lock). Both read `[X]`; A writes `[X, oxfmt]`; B — which never saw "oxfmt" — writes `[X, gated]`, and **"oxfmt" is gone permanently**: A's span is already condensed and will never be re-extracted. Dropping the losing pass is the right trade: it forfeits at most the facts from one span, whereas writing it destroys another pass's committed work.
...
- **Recall — `memories_section`:**
  ...
  - some → `# Memories` block with all facts, in **emission order from the last extraction pass** (an earlier draft said "newest-first" — impossible: a set swap gives every fact one identical `updated_at`, so there is no "newest" to sort by. Ordering falls through to `id ASC`, which reproduces insertion order because `Uuid::now_v7()` is monotonic within a millisecond — a real dependency, called out at the query).
