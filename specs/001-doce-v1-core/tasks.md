...
- [ ] T095 **Partial — rust/frontend green, e2e blocked on an unresolved CI-environment gap**: `rust` and `frontend` jobs are reliably green (confirmed across several consecutive runs, after fixing a self-inconsistent `package-lock.json` and a `@tiptap/react` test-teardown timing race — see recent commit history). `e2e` fails consistently and reproducibly in GitHub Actions, but the exact same specs against the exact same code pass reliably when run locally under an identical full data wipe (confirmed directly: `onboarding.spec.ts`'s all 4 tests passed locally in ~6.5 minutes with a real model download, right after a CI run failed the same spec). This is genuinely a CI-environment issue, not an application bug.
  ...
  Leading hypothesis given the above: some async initialization the app performs on mount (its initial hardware-detection/settings/model-status IPC call, or similar) hangs indefinitely in this specific CI environment, and the loading state renders nothing visible while pending — rather than the webview or JS engine being broken outright. Not yet root-caused further (a real IPC round-trip currently requires a genuine backend response, so this would need e.g. instrumenting the Rust side or intercepting the specific invoke call to see whether it ever arrives at all).

  Real next steps for whoever picks this up: instrument (temporarily) whichever command the app's shell/root component invokes first on mount, to log on the Rust side whether it's ever received in CI; check whether GitHub's macOS runner image needs the built `.app` to have its quarantine attribute stripped (`xattr -cr`) for the real Tauri IPC bridge (not just the wdio plugin's own layer) to initialize correctly; consider whether `@wdio/tauri-service`'s `driverProvider: "embedded"` mode has a documented GitHub Actions caveat not yet found. Given rust/frontend are solid and the application logic itself is proven correct by the extensive local e2e coverage all session, this is scoped as a CI-harness gap to track separately rather than a v1.0 release blocker on its own — but it does mean T095's "all three jobs green" bar isn't met yet.
