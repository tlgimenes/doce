...
- [ ] T095 **Partial — rust/frontend green, e2e blocked on an unresolved CI-environment gap**: `rust` and `frontend` jobs are reliably green (confirmed across several consecutive runs, after fixing a self-inconsistent `package-lock.json` and a `@tiptap/react` test-teardown timing race — see recent commit history). `e2e` fails consistently and reproducibly in GitHub Actions, but the exact same specs against the exact same code pass reliably when run locally under an identical full data wipe (confirmed directly: `onboarding.spec.ts`'s all 4 tests passed locally in ~6.5 minutes with a real model download, right after a CI run failed the same spec). This is genuinely a CI-environment issue, not an application bug — investigated across several hypotheses, each disproven or inconclusive: (1) a redundant debug-build step before the real e2e build — removed, no change; (2) CI's GPU reporting as a paravirtualized Metal device with an observed 11+ second shader-library load — bumped the earliest UI-check timeouts from 15-20s to 60s, no change (the app's own `<h1>` heading still never appears even after a full 60 seconds, which rules out "just needs more time" — something structurally prevents the webview from rendering any content at all in this specific environment); (3) checked `@wdio/tauri-service`'s own troubleshooting guide's "Tauri plugin not available" checklist (registered Rust plugin, frontend import, capabilities — all present) except `withGlobalTauri` in `tauri.conf.json`, which is unset — but this is very likely a red herring since the identical warning ("Tauri core.invoke not available") also appears in the passing local runs. Real next steps for whoever picks this up: bump `wdio.conf.ts`'s `logLevel` to `"debug"`/`"trace"` for one CI run to get more detail on the actual window-creation/navigation sequence; check whether GitHub's macOS runner image needs the built `.app` to have its quarantine attribute stripped (`xattr -cr`) for WebKit remote-automation to work at all (a known requirement for some unsigned-macOS-GUI-app CI setups); consider whether `@wdio/tauri-service`'s `driverProvider: "embedded"` mode has a documented GitHub Actions caveat not yet found. Given rust/frontend are solid and the application logic itself is proven correct by the extensive local e2e coverage all session, this is scoped as a CI-harness gap to track separately rather than a v1.0 release blocker on its own — but it does mean T095's "all three jobs green" bar isn't met yet.
