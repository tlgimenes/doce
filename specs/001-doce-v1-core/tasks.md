...
- [X] T095 **All three jobs confirmed green.** `rust`/`frontend` were already reliably green (after fixing a self-inconsistent `package-lock.json` and a `@tiptap/react` test-teardown timing race). `e2e` failed consistently and reproducibly in GitHub Actions for several rounds while passing every time locally under identical conditions — root-caused across a multi-step investigation (redundant build step removed; CI's paravirtualized-GPU slowness ruled out via generous timeout bumps that still didn't help; temporary DOM diagnostics proved the webview does navigate and does run JS but produces no visible content; tracing `src/App.tsx` directly found the actual mechanism: `if (ready === null) return null` gates the whole app behind `commands.listModels()` — its first-ever `invoke()` call — resolving, with no timeout, so a hang there means the app renders nothing forever).

  Fix shipped: `src/lib/withTimeout.ts` (races a promise against a bounded timeout, unit-tested) plus `App.tsx`'s `checkReadyWithRetries()` (3 retries, 8s each, falls back to `ready = false` — showing Onboarding — instead of hanging indefinitely), with dedicated fake-timer unit tests proving the fallback behavior.

  Verification hit an unrelated, external blocker mid-way: GitHub Actions hosted runners (`macos-26`) went account-wide billing-blocked ("recent account payments have failed"), so no push-triggered run could execute at all. Rather than wait, registered a temporary self-hosted runner on local hardware (private repo, so no external-PR risk) and routed `rust`/`frontend`/`e2e` to it for one verification pass. First attempt failed on a real but unrelated problem — the local disk was down to ~500MB free (`src-tauri/target/` had grown to ~65GB across a long session of repeated builds) — freed via `cargo clean`. Second attempt (`gh run rerun --failed`) came back **fully green**: rust ✓, frontend ✓, e2e ✓ (23m11s), confirming `checkReadyWithRetries()` genuinely resolves the failure. `ci.yml` has been reverted back to `runs-on: macos-26` for all jobs (self-hosted isn't a sustainable long-term setup) and the temporary runner fully deregistered and removed — this was a one-time verification tool, not a fixture.

  Residual open item, **external to this repo**: the GitHub Actions billing issue itself is still unresolved on the account, so the *next* push-triggered CI run on hosted runners will still fail to start until the repo owner resolves it in Settings → Billing & plans. The code-level fix and its in-CI verification are both done; only that account-level action remains, and it isn't something achievable from within this repository.
...
- **T090–T095 (release polish) — updated in a later pass**: `T090`
  ...
  repo owner), `T092` (security pass), `T093` (moot), `T094` (`README.md`),
  and `T095` (all three CI jobs confirmed green, including a real e2e fix)
  are now done. `T091` (full `quickstart.md` pass against a built binary)
  remains open. See each task's own entry above for specifics.
