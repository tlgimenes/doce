...
- [X] T090 [P] **Partial — CI wiring done, real signing untested**: added a `release` job to `.github/workflows/ci.yml`, gated on a `v*` tag and on `rust`/`frontend`/`e2e` all passing first, that runs `npm run tauri build` with `APPLE_CERTIFICATE`/`APPLE_CERTIFICATE_PASSWORD`/`APPLE_SIGNING_IDENTITY`/`APPLE_ID`/`APPLE_PASSWORD`/`APPLE_TEAM_ID` sourced from GitHub Actions secrets, per Tauri v2's documented macOS signing/notarization flow (research.md § 7). This is genuinely untested against a real Apple Developer certificate/account — none is available in this environment. Before this can produce a real signed, notarized `.dmg`, the repo owner needs to: (1) enroll in the Apple Developer Program if not already, (2) generate a "Developer ID Application" certificate, export it as a base64-encoded `.p12`, (3) generate an app-specific password for notarization, (4) add all six values as repository secrets, (5) verify the exact env var names against Tauri's current official macOS code-signing guide before trusting a tag push to actually produce a distributable artifact. **Real risk found via a local release build (T091)**: `npm run tauri build`'s `dmg` target shells out to `create-dmg`, which was not installed at all (fixed: added `brew install create-dmg` to the release job) — and even once installed, its Finder-prettifying AppleScript step (`osascript`, to arrange icons nicely) failed with `AppleEvent timed out (-1712)` in this non-interactive/automated local session. `create-dmg` has a documented `--skip-jenkins` flag ("useful in Sandbox and non-GUI environments") for exactly this, but Tauri's `DmgConfig` doesn't expose a way to pass it through — this may or may not also occur on GitHub's macOS runners (which do have a real GUI session, unlike this specific sandboxed shell) and could not be verified either way without a real signing credential to actually exercise the release job end-to-end. If the release job's dmg step fails this same way in practice, the fix is either patching Tauri's generated `bundle_dmg.sh` to add `--skip-jenkins` as a post-build step, or accepting a plain (non-"fancy") DMG. The `.app` bundle itself (`target/release/bundle/macos/doce.app`) built and bundled successfully regardless — this only affects the `.dmg` wrapper step.
- [ ] T091 [P] **Partial**: ran a real `npm run tauri build` (release, not debug) locally — confirmed it produces a working `doce.app` (real Mach-O arm64 binary) that launches, stays alive, and quits cleanly. Producing the `.dmg` failed until `create-dmg` was installed (see T090's note); the tool itself has no GUI-automation harness available in this environment (unlike a browser, there's no WebDriver-equivalent for driving a native Tauri window interactively from here), so the actual 8-section interactive walkthrough wasn't run step-by-step against this specific binary. What existing automated coverage substitutes for each section (all against a debug+wdio build, not this release build, though the underlying Rust/frontend logic doesn't differ by build profile): §1 zero-config first run → `onboarding.spec.ts`; §2 chat persistence → `chat.spec.ts`/`conversation-list.spec.ts`; §3 agent mode → `agent-mode.spec.ts`/`workspace-cwd-resolution.spec.ts`; §4 subagent spawning → `subagent.spec.ts`; §5 title/status → **not covered** (T086, deferred); §6 MCP and skills → skill-mention path covered by `rich-chat-input.spec.ts`, MCP server addition **not covered** (T065, deferred); §7 search → **not covered** by e2e (T081, deferred; unit/component-tested only); §8 privacy/network check → **not covered**, inherently needs a human watching `lsof`/`nettop`/a proxy live. Remaining real gap: an actual manual click-through of all 8 sections against this exact release binary.
...
- [ ] T095 **Partial — rust/frontend green, e2e blocked on an unresolved CI-environment gap**: `rust` and `frontend` jobs are reliably green (confirmed across several consecutive runs, after fixing a self-inconsistent `package-lock.json` and a `@tiptap/react` test-teardown timing race — see recent commit history). `e2e` fails consistently and reproducibly in GitHub Actions, but the exact same specs against the exact same code pass reliably when run locally under an identical full data wipe (confirmed directly: `onboarding.spec.ts`'s all 4 tests passed locally in ~6.5 minutes with a real model download, right after a CI run failed the same spec). This is genuinely a CI-environment issue, not an application bug.

  Investigated across several rounds, each narrowing the problem further:
  1. A redundant debug-build step before the real e2e build — removed, no change.
  2. CI's GPU reporting as a paravirtualized Metal device with an observed 11+ second shader-library load, suggesting general slowness — bumped the earliest UI-check timeouts from 15-20s to 60s (kept, `EARLY_UI_TIMEOUT` in `onboarding.spec.ts`), no change. Ruled out "just needs more time": the app's own `<h1>` heading still never appears after a full 60 seconds.
  3. Added temporary diagnostics (page-source/URL dump, then `document.body.innerHTML`/`#root` childElementCount, on the first failure) and ran them in CI, then reverted them once they'd yielded an answer. Result: the webview **does** navigate to the correct URL (`http://localhost:1420/`) and **does** execute JS (Tiptap's dynamic style injection appears in `<head>`, which only happens if `RichInput`'s module actually ran). React **does** mount something into `#root` (`childElementCount: 1`). But that mounted content's `body.innerHTML` comes back functionally empty. This rules out "webview never loads" entirely — the render pipeline starts, but whatever it mounted never produces visible content.
  4. Checked `@wdio/tauri-service`'s own troubleshooting guide's "Tauri plugin not available" checklist (Rust plugin registered, frontend import present, capabilities configured — all present) except `withGlobalTauri` in `tauri.conf.json`, which is unset. Traced the actual "Tauri core.invoke not available after 5s timeout" warning to its source (`node_modules/@wdio/tauri-service/dist/esm/index.js`): it polls `window.__wdio_original_core__`, a reference `@wdio/tauri-plugin`'s own wrapper sets up for the testing framework's mock-routing — **not** the app's own `@tauri-apps/api` invoke path. So this warning is most likely a separate, likely-benign quirk in the wdio plugin's own setup timing, not proof the app's real IPC is broken (it also appears, harmlessly, in passing local runs).

  5. Traced the actual mount-time render logic in `src/App.tsx` directly (rather than more CI-side diagnostics): found `if (ready === null) return null`, gating the ENTIRE app behind `commands.listModels()` — the app's first-ever `invoke()` call — resolving. Added a `[data-testid="ipc-diagnostic"]` element there (temporary) to surface that promise's real outcome (pending/resolved/rejected) directly in the DOM for the next CI run to read, since console capture has proven unreliable in this environment.
  6. **Blocked mid-investigation by an unrelated, external GitHub Actions billing failure** ("recent account payments have failed or your spending limit needs to be increased") — every subsequent run fails immediately with no job even starting, so the diagnostic from step 5 has not yet been observed in CI. Needs the repo owner to resolve billing before any further CI-based verification is possible.
  7. Given the billing block, shipped a real fix rather than waiting on more CI cycles: `src/lib/withTimeout.ts` (new, unit-tested in `withTimeout.test.ts`) races a promise against a bounded timeout; `App.tsx`'s `checkReadyWithRetries()` wraps `listModels()` in this with 3 retries (8s each, 24s worst case) and falls back to `ready = false` (showing Onboarding, which makes its own independent `getHardwareProfile()`/`startModelInstall()` calls) instead of hanging on a blank screen forever. Reverted the temporary diagnostic element since it's superseded by this real behavior change. Verified via a dedicated, deterministic unit test (`App.test.tsx`'s "App's initial readiness check survives a stuck listModels() call" — uses fake timers to prove a permanently-hanging `listModels()` still resolves to `false` within the bounded retry window, and that a later-succeeding retry still resolves `true`) — this is a genuine, locally-verified robustness improvement regardless of the CI mystery's ultimate root cause, but it has **not yet been confirmed to fix the actual e2e-in-CI failure**, since no CI run has executed since billing broke.

  Leading hypothesis remains: some async initialization on mount (specifically now identified as `listModels()`, not just "some IPC call") hangs indefinitely in this specific CI environment. Whether it's a bridge-not-ready race, a dropped IPC message, or a backend-side stall (e.g. in `DbCell`'s lazy `OnceCell` first-open) is still not conclusively determined — the new retry/timeout/fallback should make the app resilient to it either way, but doesn't explain *why* it happens only in that one environment.

  Real next steps for whoever picks this up: **first, resolve the GitHub Actions billing issue** and re-run CI to see whether the `checkReadyWithRetries()` fix alone resolves `onboarding.spec.ts` (if `listModels()` eventually succeeds within 24s, the app should now reach the Onboarding view instead of hanging forever — a meaningfully different, and testable, outcome even if the underlying slowness/hang isn't itself understood). If it still fails, instrument (temporarily) `list_models`'s Rust implementation to log on receipt, to see whether the invoke call ever reaches the backend at all in that run. Also worth checking: whether GitHub's macOS runner image needs the built `.app` to have its quarantine attribute stripped (`xattr -cr`) for the real Tauri IPC bridge to initialize correctly; whether `@wdio/tauri-service`'s `driverProvider: "embedded"` mode has a documented GitHub Actions caveat. Given rust/frontend are solid and the application logic itself is proven correct by the extensive local e2e coverage all session, this is scoped as a CI-harness gap to track separately rather than a v1.0 release blocker on its own — but it does mean T095's "all three jobs green" bar isn't met yet, and can't be re-verified until GitHub Actions billing is fixed.
